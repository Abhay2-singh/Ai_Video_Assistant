...
# ─── 7. Live AI Interview Engine (Shared Architecture) ──────────────────────────
...
def generate_live_question(
    ...
    """
    Generate an adaptive, realistic technical interview question based on selected domain topic.
    Dynamically generated by LLM, avoiding repeat questions.
    """
    llm = get_llm(temperature=0.4)
...
- Score: {previous_evaluation.get('score', 'N/A')}/10
- Weakness / Missing Points: {', '.join(previous_evaluation.get('missing_points', []))}
- Misconceptions: {', '.join(previous_evaluation.get('misconceptions', []))}
- Quality: {previous_evaluation.get('explanation_quality', '')}
"""

    prompt = ChatPromptTemplate.from_messages([
        (
            "system",
            """You are a Principal Software Engineer and elite technical interviewer conducting a Live Technical Mock Interview.
The interview topic/domain is: '{topic}'.
Target Difficulty: {difficulty}.

INTERVIEW GUIDELINES:
1. Difficulty Standards:
   - Beginner: Core syntax, basic terminology, fundamental data types/concepts, 'what is X and why use it'.
   - Intermediate: Internal mechanics, common libraries, memory/lifecycle, performance trade-offs, 'how does X work under the hood', why choose X over Y.
   - Advanced: Complex edge cases, concurrency/threading, high-scale architecture, subtle failure modes, tricky debugging scenarios.
2. Question Type: {question_type} (e.g. Conceptual, Why/How, Practical, Scenario-based, Debugging).
3. Adapting to Candidate:
   - If previous feedback shows weaknesses or misconceptions, adapt naturally: probe deeper, ask a follow-up, or verify foundational clarity.
   - If previous answer was strong, challenge them with a realistic practical or architectural scenario.
4. STRICT: Do NOT repeat or slightly rephrase any of these previously asked questions:
{prev_questions}

Return ONLY a valid JSON object with the exact schema:
{{
  "question": "The interview question text spoken directly to the candidate",
  "question_type": "{question_type}",
  "difficulty": "{difficulty}",
  "concept_name": "Specific sub-concept being tested (e.g. Memory Management, Indexing, Goroutines)",
  "expected_key_points": [
    "Expected key point 1 for a high-scoring answer",
    "Expected key point 2 for a high-scoring answer"
  ]
}}
Do NOT output any conversational text or markdown code fences outside the JSON object.
"""
        ),
        (
            "human",
            "Generate Question #{q_num} for domain '{topic}' at '{difficulty}' difficulty.\n{prev_feedback}"
        )
    ])
    ...
    raw = invoke_with_retry(chain, {
        ...
        "prev_questions": prev_q_str if prev_q_str else "None yet (This is Question 1).",
        "prev_feedback": prev_feedback_str if prev_feedback_str else "Starting fresh interview."
    })
    ...
    try:
        ...
        print(f"Error parsing live question JSON: {e}, raw: {raw[:150]}")

    # Fallback tailored question
    return {
        "question": f"In {topic}, could you explain the fundamental mechanics of how key components interact and the practical trade-offs involved?",
        "question_type": question_type,
        "difficulty": difficulty,
        "concept_name": f"{topic} Core Principles",
        "expected_key_points": [
            f"Explanation of {topic} core architecture",
            "Practical trade-offs and performance implications"
        ]
    }
...
def evaluate_live_answer(
    ...
    """
    Evaluate candidate's spoken or typed answer in Live AI Interview.
    Evaluates multi-dimensionally: Concept Understanding, Technical Accuracy, Clarity & Depth.
    """
    llm = get_llm(temperature=0.1)
    ...
    prompt = ChatPromptTemplate.from_messages([
        (
            "system",
            """You are an expert technical interviewer evaluating a candidate's live answer in an interview for '{topic}'.
...
EVALUATION CRITERIA:
1. Concept Understanding (1-10): Did the candidate grasp the underlying concept rather than just reciting buzzwords?
2. Technical Accuracy (1-10): Are the technical facts, mechanics, and terminology accurate?
3. Clarity & Depth (1-10): Was the explanation articulate, well-structured, and adequately detailed?
4. Missing Points: Important technical details or mechanics omitted.
5. Misconceptions: Any incorrect statements or flawed technical logic.
6. Overall Score (1-10): Fair combined interview score.
   - 8-10: Strong / Hire
   - 5-7: Partially Correct / Mixed
   - 1-4: Weak / Significant Gaps
7. Recommended Difficulty Action:
   - 'increase' if overall score >= 8
   - 'maintain' if overall score 5 to 7
   - 'decrease' if overall score < 5
...
{{
  ...
  "clarity_depth_score": 7,
  "rating": "Strong | Partially Correct | Weak",
  "correct_points": ["Specific accurate points the candidate made"],
  "missing_points": ["Key technical details the candidate should have mentioned"],
  "misconceptions": ["Any technical inaccuracies stated (empty list if none)"],
  "explanation_quality": "Brief assessment of communication clarity and technical depth",
  "feedback_summary": "2-3 sentences of direct, encouraging and actionable interview feedback",
  "recommended_difficulty_action": "increase | maintain | decrease",
  "weak_concept_identified": "Specific sub-concept or topic to reinforce, or null"
}}
Do NOT output anything outside the JSON object.
"""
        ),
        ("human", "Evaluate the candidate's answer now.")
    ])
    ...
    raw = invoke_with_retry(chain, {
        ...
        "question": question,
        "expected_points": "\n".join([f"- {p}" for p in expected_points]),
        "user_answer": user_answer.strip() if user_answer.strip() else "(No verbal or written answer was given)"
    })
    ...
    try:
        ...
        if isinstance(eval_data, dict) and "score" in eval_data:
            score = int(eval_data.get("score", 5))
            eval_data["score"] = max(1, min(10, score))
            eval_data["concept_understanding_score"] = max(1, min(10, int(eval_data.get("concept_understanding_score", score))))
            eval_data["technical_accuracy_score"] = max(1, min(10, int(eval_data.get("technical_accuracy_score", score))))
            eval_data["clarity_depth_score"] = max(1, min(10, int(eval_data.get("clarity_depth_score", score))))
        ...
        print(f"Error parsing live evaluation JSON: {e}, raw: {raw[:150]}")
    ...
    return {
        "score": fallback_score,
        "concept_understanding_score": fallback_score,
        "technical_accuracy_score": fallback_score,
        "clarity_depth_score": fallback_score,
        "rating": "Partially Correct" if fallback_score >= 5 else "Weak",
        "correct_points": ["Candidate articulated initial understanding."],
        "missing_points": ["Deep technical explanation with specific mechanisms."],
        "misconceptions": [],
        "explanation_quality": "Initial response provided.",
        "feedback_summary": "Good initial effort. Aim to provide deeper mechanics and practical trade-offs.",
        "recommended_difficulty_action": "maintain",
        "weak_concept_identified": concept_name
    }


def select_adaptive_live_next(
    topic: str,
    interview_history: List[Dict],
    last_evaluation: Dict,
    current_difficulty: str
    ...
    """
    Adaptive controller for Live AI Interview.
    Decides: (concept_focus, next_difficulty, question_type)
    """
    last_score = last_evaluation.get("score", 5)
    action = last_evaluation.get("recommended_difficulty_action", "maintain")
    next_difficulty = get_next_difficulty(current_difficulty, action)

    last_item = interview_history[-1] if interview_history else {}
    last_concept = last_item.get("concept_name", f"{topic} Core")

    # Score >= 8: Increase difficulty, shift to practical/scenario or advanced architectural topics
    if last_score >= 8:
        q_type = "Practical" if next_difficulty == "Intermediate" else "Scenario-based"
        return f"Advanced {topic} Mechanics", next_difficulty, q_type

    # Score 5-7: Maintain difficulty, probe missing points with Follow-up or Why/How
    if 5 <= last_score <= 7:
        weak_c = last_evaluation.get("weak_concept_identified") or last_concept
        return weak_c, next_difficulty, "Follow-up"

    # Score < 5: Decrease difficulty, clarify fundamental concept
    weak_c = last_evaluation.get("weak_concept_identified") or last_concept
    return f"Fundamentals of {weak_c}", next_difficulty, "Why/How"


def generate_live_final_report(
    interview_history: List[Dict],
    topic: str,
    duration_str: str = ""
) -> Dict:
    """
    Generate comprehensive final scorecard for Live AI Interview.
    """
    if not interview_history:
        return {
            "overall_score": 0.0,
    ...
    for item in interview_history:
        ev = item.get("evaluation", {})
        c_name = item.get("concept_name", "Concept")
        sc = ev.get("score", 5)
        cu = ev.get("concept_understanding_score", sc)
        ta = ev.get("technical_accuracy_score", sc)
        cd = ev.get("clarity_depth_score", sc)
        ...
        if sc >= 8 and c_name not in strong_concepts:
            strong_concepts.append(c_name)
        elif sc < 6 and c_name not in weak_concepts:
            weak_concepts.append(c_name)
    ...
    n = len(interview_history)
    overall_score = round(total_score / n, 1)
    avg_cu = round(total_cu / n, 1)
    avg_ta = round(total_ta / n, 1)
    avg_cd = round(total_cd / n, 1)
    ...
    rec_prompt = ChatPromptTemplate.from_messages([
        (
            "system",
            """You are a senior engineering mentor. Based on a candidate's live technical interview performance in '{topic}':
Generate 3 to 5 prioritized, concrete practice recommendations.
...
[
  {{
    "topic": "Concept or Skill Name",
    "action": "Concrete, actionable study or coding exercise recommendation",
    "priority": "High | Medium"
  }}
]
...
"""
        ...
        ("human", "Generate practice recommendations now.")
    ])
    ...
    try:
        ...
        print(f"Error generating live recommendations: {e}")

    if not recommended_topics:
        for wc in weak_concepts:
            recommended_topics.append({
                "topic": wc,
                "action": f"Review documentation, core patterns, and implement hands-on examples of {wc}.",
                "priority": "High"
            })
        if not recommended_topics:
            recommended_topics.append({
                "topic": f"{topic} Advanced Best Practices",
                "action": "Great performance! Build full-scale production projects and study system design trade-offs.",
                "priority": "Medium"
            })

    return {
        "overall_score": overall_score,
        "concept_understanding_score": avg_cu,
        "technical_accuracy_score": avg_ta,
        "clarity_depth_score": avg_cd,
        "strong_concepts": strong_concepts,
        "weak_concepts": weak_concepts,
        "missing_concepts": all_missing_points[:5],
        "misconceptions": all_misconceptions[:5],
        "recommended_topics": recommended_topics,
        "duration": duration_str,
        "topic": topic
